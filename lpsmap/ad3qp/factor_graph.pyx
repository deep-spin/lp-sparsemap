...
cdef class PFactorGraph:
    ...
    def create_factor_pair(self,
        ...
        pf.thisptr = self.thisptr.CreateFactorPAIR(variables,
                                                   edge_log_potential,
                                                   owned_by_graph)
        return pf
    ...
    def create_factor_knapsack(self,
        ...
        cdef PFactor pf = PFactor()
        pf.thisptr = self.thisptr.CreateFactorKNAPSACK(variables, negated_,
                                                       costs,
                                                       budget,
                                                       owned_by_graph)
        return pf
    ...
    def create_factor_dense(self, list p_multi_variables,
        ...
        cdef Factor* f
        f = self.thisptr.CreateFactorDense(multi_variables,
                                           additional_log_potentials,
                                           owned_by_graph)
        if not owned_by_graph:
            print("Deprecated behavior, owned_by_graph should be True.")
            return

        pf = PGenericFactor(allocate=False)
        pf.thisptr = f
        return pf
    ...
    def solve_qp_ad3(self):
        cdef vector[double] posteriors
        cdef vector[double] additional_posteriors
        cdef double value
        cdef int solver_status
        solver_status = self.thisptr.SolveQP(&posteriors,
                                             &additional_posteriors,
                                             &value)
        return value, posteriors, additional_posteriors, solver_status
    ...
    def jacobian_vec(self, double[:] du, double[:] out, n_iter=100, atol=1e-8):
        """Compute Jacobian-vector product Jdu of the SparseMAP solution.

        Parameters
        ----------
        du : np.array
            Vector to multiply by the Jacobian. In a neural network setting,
            this would be the gradient of the loss w.r.t. the marginals mu.

        out : np.array,
            Memory location where the result will be stored. Must have same
            shape as `du`. In a neural network setting, this would
            be the gradient of the loss w.r.t. the variables scores eta.

        Returns
        -------
        out_additional, list of lists
            Variable-shaped list of gradients w.r.t. the additional scores
            of each factor. In a neural network setting, out_additional[f] would
            be the gradient of the loss w.r.t. the factor additional scores
            (eta_f,V).
        ...
        Section 3.2 of
            ...
            LP-SparseMAP: Differentiable Relaxed Optimization for Sparse
            Structured Prediction, V. Niculae and A. F. M. Martins, 2020.
            https://arxiv.org/abs/2001.04437
        """
        return self.thisptr.JacobianVec(&du[0], &out[0], n_iter, atol)
    ...
    def solve_qp(self, eta=0.1, adapt=True, max_iter=1000, tol=1e-6,
                 ensure_multi_variables=True, verbose=False,
                 branch_and_bound=False):
        """Solve the LP-SparseMAP problem associated with the factor graph.

        Parameters
        ---------

        eta : float, default: 0.1
            Value of the ADMM penalty. If adapt_eta is true, this is the
        ...
        posteriors : list
            The SparseMAP marginal (mu) of each binarized variable in the graph,
            in the order in which they were created.  Multi-valued variables are
            represented using a value for each state.

        additional_posteriors : list
            Additional posteriors (mu_v) for each log-potential in the factors.
    ...
    def solve(self, eta=0.1, adapt=True, max_iter=1000, tol=1e-6,
        ...
        posteriors : list
            ...
            in the order in which they were created. Multi-valued variables are represented using a value for each state.  If solution is
