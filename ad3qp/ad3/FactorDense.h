...
class FactorDense : public GenericFactor
{
  public:
    ...
    void Print(ostream& stream)
    {
        ...
        for (size_t k = 0; k < multi_variables_.size(); ++k) {
            int num_states = multi_variables_[k]->GetNumStates();
            stream << " " << num_states;
        }

        // Write the additional log-potentials.
        size_t num_configurations = GetNumConfigurations();
                  ...
                  double* value)
    {
        ...
        int best = -1;
        *value = -1e12;
        size_t n_add = additional_log_potentials.size();
        for (size_t index = 0; index < n_add; ++index) {
  ...
  public:
    ...
    // num_states contains the number of states for each multi-variable linked
    // to the factor. Note: the variables and the additional log-potentials must
    // be ordered properly.
    void Initialize(const vector<MultiVariable*>& multi_variables)
    {
        ...
        for (size_t i = 1; i < multi_variables_.size(); ++i) {
            variable_offsets_[i] =
              variable_offsets_[i - 1] + multi_variables_[i]->GetNumStates();
        }
    }

    size_t GetNumMultiVariables() { return multi_variables_.size(); }
    ...
    size_t GetNumAdditionals() override {
        int num_add = 1;
        for (auto *var : multi_variables_)
            num_add *= var->GetNumStates();
        return num_add;
    }
