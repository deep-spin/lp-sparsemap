...
{
    return (((t1.tv_sec - t2.tv_sec) * 1000000000) + (t1.tv_usec - t2.tv_usec));
}
                         ...
                         vector<bool>& tight_ineq)
{
    ...
    for (long int jj = d - 1; jj >= 0; jj--) {
        s += y[jj].first;
        yav = s / ((double)(d - jj));
        if (jj == 0 || yav >= y[jj - 1].first)
            break;
    }

    for (; j < d; j++) {
        x[y[j].second] = yav;
        tight_ineq[y[j].second] = true;
    }
...
{
    size_t d = x.size();
    vector<pair<double, size_t>> y(d - 1);

    for (size_t j = 0; j < d - 1; j++) {
        y[j].first = -x[j] * sqrt_degrees[j]; // - to sort desc
        y[j].second = j;
    }
    ...
    // cumulative sums:
    double cum_ssq = 1 / degrees[d - 1];             // rho^2 = 1 / deg
    double cum_ssr = x[d - 1] / sqrt_degrees[d - 1]; // rho * x = x / sqrt(deg)

    size_t jj;

    double tau = cum_ssr / cum_ssq;

    for (size_t j = 0; j < d - 1; j++) {
        if (tau >= -y[j].first)
            break;

        jj = y[j].second; // sorted index
        cum_ssq += 1 / degrees[jj];
        cum_ssr += x[jj] / sqrt_degrees[jj];
        tau = cum_ssr / cum_ssq;
    }

    // can probably be sped up like above, but no matter
    for (size_t j = 0; j < d - 1; j++) {
        if (x[j] >= tau / sqrt_degrees[j]) {
            x[j] = tau / sqrt_degrees[j];
            tight_ineq[j] = true;
        }
    }

    x[d - 1] = tau / sqrt_degrees[d - 1];
    tight_ineq[d - 1] = true;
    return 0;
}
...
int
project_onto_knapsack_constraint(double* x, double* costs, size_t d, double budget)
{
    ...
    */
    ...
    solve_canonical_qp_knapsack(
      lower_bounds, upper_bounds, weights, total_weight, &solution);
                              ...
                              const vector<double>& sqrt_degrees)
{
    ...
    size_t d = x.size();
    vector<double> lower(d), upper(d), out(d);
    double total = -1;

    for (size_t j = 0; j < d; ++j) {
        total += sqrt_degrees[j] * x[j];
        upper[j] = x[j] / sqrt_degrees[j];
        lower[j] = upper[j] - (1 / degrees[j]);
    }

    int rc = solve_canonical_qp_knapsack(lower, upper, degrees, total, &x);
    for (size_t j = 0; j < d; ++j) {
        x[j] -= upper[j];
        x[j] *= -sqrt_degrees[j];
    }
...
{
    size_t d = x.size();
    vector<double> lower(d), upper(d), out(d), c(d);

    double total = -budget;
    for (size_t j = 0; j < d; ++j) {
        c[j] = degrees[j] * costs[j] * costs[j];
        total += x[j] * costs[j] * sqrt_degrees[j];
        upper[j] = x[j] / (costs[j] * sqrt_degrees[j]);
        lower[j] = upper[j] - (1 / (costs[j] * degrees[j]));
    }

    int rc = solve_canonical_qp_knapsack(lower, upper, c, total, &x);
    for (size_t j = 0; j < d; ++j) {
        x[j] -= upper[j];
        x[j] *= -costs[j] * sqrt_degrees[j];
    }
                            ...
                            vector<double>* solution)
{
    ...
    vector<pair<double, size_t>> sorted_lower(d);
    vector<pair<double, size_t>> sorted_upper(d);
    for (size_t i = 0; i < d; ++i) {
    ...
    while (k < d || l < d) {
        ...
        //if (!(level == 0 || tau >= left)) {
            // std::cout << "assertion would fail" << tau << ">=" << left << std::endl;
            // tau = left;
        //}
